...
public class TiposDespesas implements Serializable {
	...
	public List<TipoLancamento> todos() {
		return this.manager.createQuery("from TipoLancamento where id != 7 order by descricao", TipoLancamento.class).getResultList();
	}
	...
	public List<TipoLancamento> filtrados(TipoLancamentoFilter filter) {
		return this.manager.createQuery("from TipoLancamento i where i.id != 7 AND i.descricao like :descricao order by descricao", TipoLancamento.class)
